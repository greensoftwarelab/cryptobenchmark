...
public class MeasureSymmetricDecryptTest {

    private static int N_EXECS = 1;
...
*/
    ...
    public void test_AES_GCM_NoPadding_AndroidKeyStore() {
        ...
        SecretKey sk = gen_key_AES_AndroidKeyStore(256, mode, padd);
        IvParameterSpec iv = new IvParameterSpec(new byte[8]);
        for (int i = 0; i < N_EXECS ; i++) {
            String target = msg + (i % 9);
            decrypt_AES(target, mode, padd, sk, provider, iv);
        }
    }
    ...
    public void test_AES_GCM_NoPadding_AndroidOpenSSL() {
        ...
        SymmetricEncrypt se = new SymmetricEncrypt(new DeviceCryptoPrimitives());
        SecretKey sk = gen_key_AES_AndroidOpenSSL(256, mode, padd);
        for (int i = 0; i < N_EXECS ; i++) {
            String target = msg + (i % 9);
